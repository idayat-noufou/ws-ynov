const pg = require('pg');

const config = {
    user: "avnadmin",
    password: "AVNS_2NI8qMCtaH2oGsCs3ib",
    host: "pg-12cfbda4-ynov-ws-jin-pg.a.aivencloud.com",
    port: 27541,
    database: "defaultdb",
    ssl: {
        rejectUnauthorized: true,
        ca: `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,
    },
};

let sql = "CREATE TABLE ws_masks (id INT PRIMARY KEY,name VARCHAR(50),description VARCHAR(255),mask_json VARCHAR(255));";
sql += "CREATE TABLE ws_entries (id INT PRIMARY KEY,mask_id INT,entry_json VARCHAR(255),FOREIGN KEY (mask_id) REFERENCES ws_masks(id))";

const client = new pg.Client(config);
client.connect(function (err) {
    if (err)
        throw err;
    client.query(sql, [], function (err, result) {
        if (err)
            throw err;

        console.log("les tables ws_masks et ws_entries ont été crées.");
        client.end(function (err) {
            if (err)
                throw err;
        });
    });
});
